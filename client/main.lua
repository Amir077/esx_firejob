			...
			if Config.MaxInService ~= -1 then
				ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
					if isInService then
						...
						TriggerEvent('esx_firejob:updateBlip')
...
function OpenVehicleSpawnerMenu(type, station, part, partNum)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle', {
		...
		if data.current.action == 'buy_vehicle' then
			...
			if type == 'car' then
				shopCoords = Config.FireStations[station].Vehicles[partNum].InsideShop
			...
			elseif type == 'helicopter' then
				shopCoords = Config.FireStations[station].Helicopters[partNum].InsideShop
...
function StoreNearbyVehicle(playerCoords)
	...
	ESX.TriggerServerCallback('esx_firejob:storeNearbyVehicle', function(storeSuccess, foundNum)
...
function GetAvailableVehicleSpawnPoint(station, part, partNum)
	local spawnPoints = Config.FireStations[station][part][partNum].SpawnPoints
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop_confirm', {
			...
			if data2.current.value == 'yes' then
				...
				ESX.TriggerServerCallback('esx_firejob:buyJobVehicle', function (bought)
					if bought then
...
function OpenVehicleInfosMenu(vehicleData)
	ESX.TriggerServerCallback('esx_firejob:getVehicleInfos', function(retrivedInfo)
...
function OpenBuyWeaponsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_buy_weapons', {
		...
		if data.current.hasWeapon then
			...
			ESX.TriggerServerCallback('esx_firejob:buyWeapon', function(bought)
				if bought then
...
AddEventHandler('esx:setJob', function(job)
	...
	Citizen.Wait(5000)
	TriggerServerEvent('esx_firejob:forceBlip')
end)
...
AddEventHandler('esx_phone:cancelMessage', function(dispatchNumber)
	if PlayerData.job and PlayerData.job.name == 'fire' and PlayerData.job.name == dispatchNumber then
...
AddEventHandler('esx_firejob:hasEnteredEntityZone', function(entity)
	...
	if PlayerData.job and PlayerData.job.name == 'fire' and IsPedOnFoot(playerPed) then
...
AddEventHandler('esx_firejob:OutVehicle', function()
	local playerPed = PlayerPedId()
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and PlayerData.job and PlayerData.job.name == 'fire' then
		...
		if IsControlJustReleased(0, 167) and not isDead and PlayerData.job and PlayerData.job.name == 'fire' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'fire_actions') then
			if Config.MaxInService == -1 then
				OpenFireActionsMenu()
			elseif playerInService then
				OpenFireActionsMenu()
...
RegisterNetEvent('esx_firejob:updateBlip')
AddEventHandler('esx_firejob:updateBlip', function()
...
AddEventHandler('playerSpawned', function(spawn)
	...
	if not hasAlreadyJoined then
		TriggerServerEvent('esx_firejob:spawned')
	end
