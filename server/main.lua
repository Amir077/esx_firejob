...
AddEventHandler('esx_firejob:confiscatePlayerItem', function(target, itemType, itemName, amount)
	...
	if sourceXPlayer.job.name ~= 'fire' then
		print(('esx_firejob: %s attempted to confiscate!'):format(xPlayer.identifier))
		return
	end
...
AddEventHandler('esx_firejob:drag', function(target)
	...
	if xPlayer.job.name == 'fire' then
		TriggerClientEvent('esx_firejob:drag', target, source)
	else
		print(('esx_firejob: %s attempted to drag (not cop)!'):format(xPlayer.identifier))
	end
end)
...
AddEventHandler('esx_firejob:putInVehicle', function(target)
	...
	if xPlayer.job.name == 'fire' then
		...
		print(('esx_firejob: %s attempted to put in vehicle (not cop)!'):format(xPlayer.identifier))
	end
end)
...
AddEventHandler('esx_firejob:OutVehicle', function(target)
	...
	if xPlayer.job.name == 'fire' then
		...
		print(('esx_firejob: %s attempted to drag out from vehicle (not cop)!'):format(xPlayer.identifier))
	end
end)
...
ESX.RegisterServerCallback('esx_firejob:getVehicleFromPlate', function(source, cb, plate)
	MySQL.Async.fetchAll('SELECT owner FROM owned_vehicles WHERE plate = @plate', {
		['@plate'] = plate
	}, function(result)
...
ESX.RegisterServerCallback('esx_firejob:addArmoryWeapon', function(source, cb, weaponName, removeWeapon)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('esx_firejob:buyWeapon', function(source, cb, weaponName, type, componentNum)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if not selectedWeapon then
		print(('esx_firejob: %s attempted to buy an invalid weapon.'):format(xPlayer.identifier))
		cb(false)
	else
		-- Weapon
		if type == 1 then
		...
		elseif type == 2 then
			...
			if component then
				...
				print(('esx_firejob: %s attempted to buy an invalid weapon component.'):format(xPlayer.identifier))
				cb(false)
			end
		end
	end
...
ESX.RegisterServerCallback('esx_firejob:buyJobVehicle', function(source, cb, vehicleProps, type)
	...
	if price == 0 then
		print(('esx_firejob: %s attempted to exploit the shop! (invalid vehicle model)'):format(xPlayer.identifier))
		cb(false)
	else
		if xPlayer.getMoney() >= price then
			xPlayer.removeMoney(price)
...
ESX.RegisterServerCallback('esx_firejob:storeNearbyVehicle', function(source, cb, nearbyVehicles)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if not foundPlate then
		...
		MySQL.Async.execute('UPDATE owned_vehicles SET `stored` = true WHERE owner = @owner AND plate = @plate AND job = @job', {
			...
			if rowsChanged == 0 then
				print(('esx_firejob: %s has exploited the garage!'):format(xPlayer.identifier))
				cb(false)
			else
				cb(true, foundNum)
			end
...
AddEventHandler('playerDropped', function()
	...
	if _source ~= nil then
		...
		if xPlayer ~= nil and xPlayer.job ~= nil and xPlayer.job.name == 'fire' then
			Citizen.Wait(5000)
			TriggerClientEvent('esx_firejob:updateBlip', -1)
		end
	end
end)

RegisterServerEvent('esx_firejob:spawned')
AddEventHandler('esx_firejob:spawned', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
RegisterServerEvent('esx_firejob:forceBlip')
AddEventHandler('esx_firejob:forceBlip', function()
	TriggerClientEvent('esx_firejob:updateBlip', -1)
end)
...
RegisterServerEvent('esx_firejob:message')
AddEventHandler('esx_firejob:message', function(target, msg)
